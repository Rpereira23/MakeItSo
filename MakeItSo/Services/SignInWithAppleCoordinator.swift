        ...
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            Auth.auth().currentUser?.link(with: credential, completion: { (authresult, error) in
                if let error = error, (error as NSError).code == AuthErrorCode.credentialAlreadyInUse.rawValue {
                    ...
                    if let updatedCredentials =  (error as NSError).userInfo[AuthErrorUserInfoUpdatedCredentialKey] as? OAuthCredential {
                        print("updating credentials...")
                        Auth.auth().signIn(with: updatedCredentials) { (authResult, error) in
